...
// ===== PWA Install Logic (Sesuai Permintaan) =====
function updateInstallButtonVisibility() {
  if (!installBtn) return;
  // Tombol selalu muncul jika aplikasi BELUM terinstall
  if (isAppInstalled()) {
...
window.addEventListener('beforeinstallprompt', (e) => {
  e.preventDefault();
  deferredPrompt = e;
  // Tombol tetap muncul, tidak ada perubahan tampilan
});
...
if (installBtn) {
  installBtn.addEventListener('click', async () => {
    if (deferredPrompt) {
      ...
      // Jika prompt belum tersedia, beri instruksi manual (fallback)
      alert('Untuk install aplikasi:\n- Android: Tap menu "Tambahkan ke Layar Utama"\n- Desktop: Klik ikon "Install" di address bar\n- iOS: Tap "Bagikan" > "Tambahkan ke Layar Utama"');
    }
  });
}
...
// Deteksi perubahan mode standalone
window.matchMedia('(display-mode: standalone)').addEventListener('change', updateInstallButtonVisibility);

// Panggil saat load untuk memastikan tombol tampil jika belum install
